...
function Sobre() {
  useEffect(() => { document.title = "Sobre | Winget Store" }, [])
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <main className="max-w-3xl mx-auto px-8 py-12">
        ...
        {/* Header da página */}
        <div className="mb-10">
          <span className="text-xs font-mono text-blue-400 tracking-widest uppercase">// sobre</span>
          <h1 className="text-4xl font-bold text-white mt-2 mb-3">O projeto</h1>
          <p className="text-gray-400">Conheça o Winget Store e quem está por trás dele.</p>
        </div>

        {/* Cards de info */}
        <div className="flex flex-col gap-6 mb-8">
          {cards.map((c) => (
            <div key={c.title} className="bg-gray-800 rounded-xl p-6 border border-gray-700 hover:border-blue-600 transition-colors">
        ...
        <div className="bg-gray-800 rounded-xl p-6 border border-gray-700 mb-8">
          ...
          <div className="flex flex-wrap gap-2">
            {techs.map((t) => (
              <span key={t.name} className={`${t.color} text-white text-sm px-3 py-1 rounded-full font-medium`}>
                {t.name}
              </span>
            ))}
          </div>
